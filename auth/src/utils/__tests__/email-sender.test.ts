...
it('should throw and error when sending an email if the api is not set', async () => {
  const emailSender = EmailSender.getInstance();
...
it('should send the email correctly if the EmailSender is active and the EmailApi set', async () => {
  const emailSender = EmailSender.getInstance();
